...
extern int lineNo; //current position in the source file
extern int colNo;  // updated by readChar()
extern int currentChar;

extern CharCode charCodes[];
int state;
int ln, cn; // the postion where the current token begins
char str[MAX_IDENT_LEN];
char c;
//int checkKeyWord;
/***************************************************************/
Token* getToken(void) 
{   //printf("getToken: state=%d, currentChar=%c (%d)\n", state, currentChar, currentChar);
    Token *token;
    switch(state)
    {
    case 0:
    ...
    // TODO Recognize Identifiers and keywords
      ...
      readChar();
      int i = 1;
      while (i < MAX_IDENT_LEN && (charCodes[currentChar] == CHAR_LETTER || charCodes[currentChar] == CHAR_DIGIT)) {
    ...
    case 4: 
        ...
        //token->tokenType = checkKeyword(str); (remove this because the token in here is an uninitialized pointer in the first version to make the (SEGMENTATION FAULT))
        //if (checkKeyWord == TK_NONE) state=5; else state =6;
        if (checkKeyWord == TK_NONE) state=5; else state =6;
        return getToken();
    case 5:
        //TODO Identifiers 
    ...
    //TODO  Numbers
      ...
      string[idx] = '\0';
      char strNumber[11];
      sprintf(strNumber, "%d", INT_MAX); // check that if the number exceeds INT_MAX
    ...
    case 24: 
      //TODO 
      readChar();
      if(currentChar != EOF && charCodes[currentChar] == CHAR_RPAR) state = 25;
      else state = 26;
      return getToken();
    case 25:
      //TODO 
    ...
    case 28:
      ...
      readChar();
      if(currentChar == '=') state = 29; else state = 30;
      return getToken();
    case 29:
    //TODO 
    ...
    case 38:
      ...
      if(currentChar == EOF) state = 40;
      else if (charCodes[currentChar] == CHAR_RPAR) state = 39;
      else state = 37;
      return getToken();
    case 39: 
      //TODO 
